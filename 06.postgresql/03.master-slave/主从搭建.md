...
2. 主库 61 创建用户
```shell
psql -c "CREATE ROLE repl_user WITH LOGIN PASSWORD '123456' REPLICATION;"
```
...
插入数据验证
```shell
psql -c "insert into test_sync values (2, 'switch test');"
...
rm -rf /data/5432/data/*
cd /data/5432
sudo -iu postgres pg_basebackup -h 10.0.0.62 -p 5432 -U repl_user -D /data/5432/data -F p -P -R -l backup20260419
# 查看会产生一个标识文件：standby.signal
ll /data/5432/data/standby.signal
# 增加连接信息
sed -i "s/#primary_conninfo/primary_conninfo/" /data/5432/data/postgresql.conf
...
# 旧主库 61 查看数据验证
psql -c "select * from test_sync;"
psql -c "insert into test_sync values (3, 'switch back test');"
# 从库执行
psql -c "select * from test_sync;"
psql -c "SELECT pg_is_in_recovery();"
# 主库执行
