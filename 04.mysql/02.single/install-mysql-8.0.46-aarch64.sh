...
# 二进制安装
cd /usr/local || exit
if [[ -f mysql-8.0.46-linux-glibc2.28-$arch.tar.xz ]]; then
    print_colored "$GREEN" "[Success] MySQL is downloaded!"
else
    print_colored "$GREEN" "[Success] Downloading MySQL..."
    wget https://cdn.mysql.com//Downloads/MySQL-8.0/mysql-8.0.46-linux-glibc2.28-$arch.tar.xz
    print_colored "$GREEN" "[Success] Downloading completed!"
fi

# 解压
tar xvf mysql-8.0.46-linux-glibc2.28-$arch.tar.xz
ln -s mysql-8.0.46-linux-glibc2.28-$arch mysql
